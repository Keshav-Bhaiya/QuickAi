...
const Sidebar = ({sidebar, setSidebar}) => {
  ...
  return (
    <div className={`w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute max-sm:top-14 max-sm:bottom-0 max-sm:z-10
    ${sidebar ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
      <div className='my-7 w-full'>
        ...
        <div className='mt-6  px-6 text-sm text-gray-600 font-medium'>
          {navItems.map((item) => (
            <NavLink 
              key={item.to} 
              to={item.to} 
